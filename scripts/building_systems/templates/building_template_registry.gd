...
func register_template(template: BuildingTemplateDefinition) -> bool:
    ...
    # Update indices
    _category_index.append_to_array(template.template_category, name)
    _style_index.append_to_array(template.architectural_style, name)
...
func _register_template_resources():
    # Factory template
    var factory_template = load("res://resources/building_templates/industrial_factory.tres")
    if factory_template:
        register_template(factory_template)
    
    # Castle template  
    var castle_template = load("res://resources/building_templates/medieval_castle.tres")
    if castle_template:
        register_template(castle_template)
    
    # Stone cottage template
    var stone_cottage_resource = load("res://resources/building_templates/stone_cottage_classic.tres")
    if stone_cottage_resource:
        register_template(stone_cottage_resource)
    
    # Thatched cottage template
    var thatched_cottage_resource = load("res://resources/building_templates/thatched_cottage.tres")
    if thatched_cottage_resource:
        register_template(thatched_cottage_resource)

# Utility class for dictionary array operations
class DictionaryArrayHelper:
    static func append_to_array(dict: Dictionary, key: String, value):
        if not dict.has(key):
            dict[key] = []
